...
function rd_rest_update_refeicao( WP_REST_Request $req ) {
    ...
    if ( $action === 'editar' ) {
        ...
        $dup = (int) $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$table} WHERE matricula=%s AND data_refeicao=%s AND id<>%d",
            $row['matricula'], $new_data, $id
        ));
        if ( $dup ) {
            return new WP_Error('rd_conflict', 'Já existe um pedido para esta data.', ['status' => 409]);
        }
