...
?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class($post_classes); ?>>
		<?php if (has_post_thumbnail() && !is_single()) : ?>
            ...
            Woffice_Frontend::render_featured_image_single_post($post->ID, 150);
            ?>
		...
		if ( has_post_thumbnail() ) {			
			...
			if ( in_category('acontece-na-btp') || in_category('eventos-btp') ) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' );
				if(isset($custom['link_imagem_destacada'])) {
					$link_imagem_destacada = $custom['link_imagem_destacada'][0];				
					$imagem_base = "<div style='text-align:center'><a href='".$link_imagem_destacada."' style='cursor:pointer'><img style='cursor:pointer' src='".$image[0]."' /></a></div>"; 
				}else {
					$imagem_base = "<div style='text-align:center'><img src='".$image[0]."' /></div>";
				}
		...
		<div class="intern-padding clearfix">
			...
			<div class="blog-sum-up">
				...
				<?php if (is_single() && $post->post_type != "pec-events"){ echo $imagem_base; }?>
			...
			<?php if (!is_single()): ?>
  			...
  			<?php 
            	...
            	//if(is_single() && $post->post_type != "pec-events") {
	            
				?>
				<div class="clearfix" style="margin-bottom: 35px;"></div>
				<div class="blog-next-page center animate-me fadeInUp" role="navigation">
				<a class="btn btn-default" href="javascript:window.history.back()" rel="prev"><i class="fa fa-hand-o-left"></i> Voltar</a>
				
				</div>
					
				<?php // }
				// DISPLAY THE NAVIGATION
            	//if($post->post_type == "pec-events"){ woffice_post_nav(); } ?>
